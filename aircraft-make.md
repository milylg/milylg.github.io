...
使用lora通信模块进行测试发现pico和esp32的cpu频率不一样导致接受和发送数据轮询有耗时差异，会导致接受端（处理速度慢的pico）缓存了大量已接受未处理的数据，这样时间一长缓冲区会溢出，再者处理延迟是不可接受的。在测试过程中，lora的引脚可能由于松动的原因有时未能完成轮询接受或者发送，重新插入引脚即可。