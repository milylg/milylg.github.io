...
2025年12月，修改风格样式，优化使用体验。

- 改变epub中css文件样式以及封面风格，删除首页顶部的过滤器名称文本的显示动画。
- 使用体验优化：编辑界面的锚点移动到编辑末尾，顶部图片自动收缩起来。
- 优化白天/夜间自动模式，根据输入或者默认经纬度和当前的阳历日期自动调整日出日落时间进行白天夜间转换。
...
- 搜索输入关闭后自动复位过滤器。
- 调整文本编辑器的时间格式。
- 删除编辑器中对外图片编码相关的接口，重新设计编辑工具栏，减少操作次数。
- 修改笔记本列表布局； 调整附件列表风格。
